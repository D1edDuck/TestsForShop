...
async function main() {
  ...
  const maxProductId = await prisma.product.aggregate({ _max: { id: true } });
  const nextId = (maxProductId._max.id || 0) + 1;
  await prisma.$executeRawUnsafe(
    `SELECT setval(pg_get_serial_sequence('"Product"', 'id'), ${nextId}, false);`,
  );
  ...
  if (john && jane) {
    const reviews = [
      { text: "Отличный телефон! Камера просто бомба.", rating: 5, userId: john.id, productId: 1 },
      { text: "Лучший планшет для работы и учёбы.", rating: 4, userId: jane.id, productId: 4 },
      { text: "Шумоподавление на высоте, очень удобные.", rating: 5, userId: john.id, productId: 3 },
    ];
